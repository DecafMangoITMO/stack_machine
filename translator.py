...
def translate_section_data(section_data: str, address: int, variables: dict[str, Variable]) -> tuple[dict[str, Variable], int]:
...
def translate_section_text_stage_2(section_text: str, labels: dict[str, int], variables: dict[str, Variable], address: int):
    lines = section_text.splitlines();
...
def translate_source(source: str) -> tuple[str, str]:
    ...
    commands = translate_section_text_stage_2(section_text, labels, variables, section_text_address)

    char_for_index = len(str(section_text_address + 2 * len(commands))) # Нужно для выравнивания индексов машинных слов в коде с комментариями
    ...
    for variable in variables.values():
        ...
        for cell in variable.data:
            if variable.is_string:
                ...
                commented_code.append(f"{address}{" " * (char_for_index - len(str(address)))} {value_to_binary32(cell)} '{chr(cell)}'")
            else:
                code.append(value_to_binary32(cell))
                commented_code.append(f"{address}{" " * (char_for_index - len(str(address)))} {value_to_binary32(cell)} {cell}")
            address += 1
    for command in commands:
        code.append(command_to_binary32(command))
        commented_code.append(f"{address}{" " * (char_for_index - len(str(address)))} {command_to_binary32(command)} {command.opcode.mnemonic}")
        address += 1
        if command.operand !=  None:
            code.append(value_to_binary32(command.operand))
            commented_code.append(f"{address}{" " * (char_for_index - len(str(address)))} {value_to_binary32(command.operand)} {command.operand}")
            address += 1
...
def main(source, target):
    ...
    write_commented_code("commented_" + target[:target.find(".")] + ".txt", commented_code)
